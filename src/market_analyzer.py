...
class MarketAnalyzer:
    ...
    def _get_report_language(self) -> str:
        """
        Resolve report language — reads directly from OS env first,
        which is guaranteed to be set by GitHub Actions workflow.
        Falls back to config, then defaults to 'zh'.
        """
        _lang = os.environ.get("REPORT_LANGUAGE", "").strip().lower()
        if _lang in ("en", "english", "en-us", "en_us"):
            return "en"
        # Fall back to config
        config_lang = getattr(self.config, 'report_language', 'zh') or 'zh'
        if config_lang.strip().lower() in ("en", "english", "en-us", "en_us"):
            return "en"
        return "zh"
...
"""
        ...
        # A 股场景：检查 report_language 决定用中文还是英文 prompt
        report_language = self._get_report_language()
        logger.info(f"[大盘] report_language resolved to: {report_language}")
